...
| NHAN_VIEN | Nhân viên | Giáo viên, cán bộ, nhân viên - có quyền xem và tạo đơn |
...
│   ┌─────────────────────────────────────────────────────────────┐ │
│   │                        ADMIN                                 │ │
│   │   Quản trị hệ thống - toàn quyền CRUD, duyệt, chốt         │ │
│   └─────────────────────────────────────────────────────────────┘ │
│                               │ quản lý                           │
│                               ▼                                   │
│   ┌─────────────────────────────────────────────────────────────┐ │
...
│   │   Quyền: Xem thông tin cá nhân, tạo đơn, check-in/out QR   │ │
│   └─────────────────────────────────────────────────────────────┘ │
│                                                                   │
└───────────────────────────────────────────────────────────────────┘
```
...
| Trạng thái | Mã | Mô tả |
|-------------|-----|-------|
| Chờ duyệt | `cho_duyet` | Đơn mới tạo, chờ Admin duyệt |
| Đã duyệt | `da_duyet` | Admin đã phê duyệt |
| Từ chối | `tu_choi` | Admin từ chối |
...
class DonXinNghi:
    ...
    # Trạng thái (1 cấp - Admin duyệt)
    trang_thai: str           # cho_duyet | da_duyet | tu_choi | huy
    
    # Phê duyệt (Admin)
    nguoi_duyet_id: str | None      # ID người duyệt
    ngay_duyet: datetime | None     # Thời gian duyệt
...
| 4 | Chỉ duyệt khi `trang_thai = cho_duyet` | Use case |
| 5 | Chỉ hủy khi `trang_thai = cho_duyet` | Use case |
| 6 | `phep_nam_con_lai >= 0` | Trigger/Use case |
...
│                                                                             │
│  1. Hệ thống tổng hợp dữ liệu check-in/out trong tháng                    │
│     → Trạng thái: chua_chot                                               │
│                                                                             │
│  2. Admin chốt chấm công                                                   │
...
PERMISSION_CODES = {
    ...
    "cham_cong:manage": "Quản lý chấm công (tạo QR, chốt, báo cáo)",
...
ROLE_PERMISSIONS = {
    ...
    "NHAN_VIEN": [
        ...
        "luong:view_own",
        # Chấm công
        "cham_cong:view_own", "cham_cong:check_in",
        # Dashboard
        "dashboard:view_employee",
        # Tài liệu
        "tai_lieu:read", "tai_lieu:create",
    ],
}
```
