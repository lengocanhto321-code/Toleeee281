...
function LoadingFallback() {
  return (
    <div className="flex items-center justify-center py-20">
      <div className="text-sm text-muted-foreground/70">Đang tải thông tin nhân viên...</div>
    </div>
  )
}
...
export default function NhanVienDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: luong } = useLuongHienTai(nhanVien?.id || "")
  const { data: kyLuongData } = useKyLuongList({ page_size: 100 })
  
  const recentKyLuong = (kyLuongData as any)?.data?.[0]
  const recentKyLuongId = recentKyLuong?.id || ""
  const { data: traLuongData } = useTraLuongByKyLuong(recentKyLuongId, { page_size: 20 })
  
  const traLuongs: TraLuong[] = (traLuongData as any)?.data?.filter(
    (tl: any) => tl.nhan_vien_id === nhanVien?.id
  ) || []

  const { data: nguoiThans } = useNguoiThanList(nhanVien?.id || "")
  const { data: bangCaps } = useBangCapList(nhanVien?.id || "")
  const { data: khenThuongKyLuats } = useKhenThuongKyLuatList(nhanVien?.id || "")
  const { data: congTacsData } = useCongTacList(nhanVien?.id || "")
  const congTacs = congTacsData?.items || []
  const { data: lichSuChucVuData } = useLichSuChucVuList(nhanVien?.id || "")
  const lichSuChucVus = lichSuChucVuData?.items || []
  const { data: hopDongsData } = useHopDongList(nhanVien?.id || "")
  const hopDongs = hopDongsData || []
  const { data: chucVus } = useChucVuList()

  const chucVuMap = useMemo(() => {
    const map: Record<string, { id: string; ten_chuc_vu: string }> = {}
    chucVus?.forEach((cv) => { map[cv.id] = { id: cv.id, ten_chuc_vu: cv.ten_chuc_vu } })
    return map
  }, [chucVus])
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append("nhan_vien_id", nhanVien.id)
      ...
      if (avatarUrl) {
        updateMutation.mutate(
          { id: nhanVien.id, data: { anh_dai_dien: avatarUrl } as Partial<NhanVien> },
  ...
  const handleTransfer = async (data: { phong_ban_id: string; chuc_vu_id: string; ngay_chuyen: string; ly_do: string }) => {
    try {
      await apiGateway.post(ApiEndpoints.CONG_TAC_CREATE(nhanVien?.id || id), {
        phong_ban_id: data.phong_ban_id,
        chuc_vu_id: data.chuc_vu_id,
        ngay_bat_dau: data.ngay_chuyen,
        is_primary: true,
        ghi_chu: data.ly_do,
      })
      toastSuccess("Thành công", "Điều chuyển nhân viên thành công")
      setTransferOpen(false)
      queryClient.invalidateQueries({ queryKey: ["nhan-vien"] })
      queryClient.invalidateQueries({ queryKey: ["cong-tac"] })
    } catch {
      toastError("Lỗi", "Điều chuyển thất bại")
    }
  }
      ...
      <AuthenticatedLayout breadcrumbLabel={nhanVien ? `${nhanVien.ho_ten} - ${nhanVien.ma_nhan_vien}` : undefined}>
        {(isLoading || !nhanVien) ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            <p className="text-sm text-muted-foreground/70">Không tìm thấy nhân viên</p>
          </div>
        ) : (
          <>
            <NhanVienDetailInfo 
              ...
              hopDongs={hopDongs}
              congTacs={congTacs}
              lichSuChucVus={lichSuChucVus}
              chucVuMap={chucVuMap}
              onEdit={handleEdit}
              onAvatarChange={handleAvatarChange}
              onLuongCreated={() => queryClient.invalidateQueries({ queryKey: ["luong"] })}
            />
